...
START_TEST(check_line_test){
	//arrays used for checks
  	int xs[] = {X, BLANK, X, O, X, BLANK};
  	int xs_2[] = {X, BLANK, X, O, O, BLANK};
  	
  	//checks for the function
  	ck_assert_int_eq(check_line(xs, 2, 3), X);
  	ck_assert_int_eq(check_line(xs_2, 2, 3), BLANK); //this ensures that every element in the section of the array (since O is at the "edge" of the section makes sure it is not returned as true with X winning when it is not true). 
